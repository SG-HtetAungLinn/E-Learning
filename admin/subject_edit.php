...
if (!$id) {
    $url = $admin_base_url . "subject_list.php?error=ID not found.";
    header("Location: $url");
    exit;
}
$error = false;
$name =
    $name_eror = '';
$res = selectData('subjects', $mysqli, "name", "WHERE `id`='$id'");
if ($res->num_rows === 0) {
    $url = $admin_base_url . "subject_list.php?error=ID not found.";
    header("Location: $url");
    exit;
...
if (isset($_POST['form_sub']) && $_POST['form_sub'] == 1) {
    ...
    if (!$error) {
        ...
        $res = updateData('subjects', $mysqli, $data, $where);
        if ($res) {
            $url = $admin_base_url . "subject_list.php?success=Subject Update Success";
            header("Location: $url");
            exit;
        }
    }
    ...
    <div class="container-xxl flex-grow-1 container-p-y">
        ...
        <div class="d-flex justify-content-center">
            <div class="col-md-8 col-lg-6">
                <div class="card">
                    <div class="card-body">
                        <form action="<?= $admin_base_url . "subject_edit.php?id=" . $id ?>" method="POST">
